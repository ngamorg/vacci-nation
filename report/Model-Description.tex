...
NETWORK
The hierarchical configuration model in STEGEHUIS ET AL provides an orientation for the model which is employed in this thesis. Adopting the two belief-groups, these are identified as sub-communities of the whole population. Hence, each group is assumed to have a close intra-connection and a looser inter-connection. A small-world model is employed by formulating Watts-Strogatz networks (WATTS STROGATZ) for each groups, which are then connected as outlined below. 
Each community \(G\in{\{T,S\}}\) is a subset of the whole population \(N\), thus \(G\subset{N}\), with T representing 'trusters' and S 'scepticals'.
\begin{equation}
...
The groups consist of a total of \(n_{m}\) members, according to \(n_{m}=\sum_{i\in{N}}p_{m}(i)=p_{m}(i)*n\). With \(n={\[\|N\|}\) being the number of nodes in the total population \(N\). Furthermore, \(p_{m}(i)\) defines the probabilistic group allocation for a node \(i\) to be assigned to group \(m\in{\{1,2\}}\), such that 
\begin{equation} 
\begin{align*}
\(p_{m}(i)=\left\{
...
\end{align*}
\end{equation}
and \(p_{m}(i)\) is independent of the individual agent \(i\). \\
The Watts-Strogatz graph for every group \(G\subset{N}\) is then built by taking into account \(k(i)\), the number of nearest neighbours OR of \(i\) that \(i\) is connected to in a ring topology. \\ 

Thereafter, the two graphs are combined by adding fraction of edges of the edges in both groups to any two members \(i,j\in{N}\) of different groups, requiring \(g(i)\neq g(j)\). The amount of edges \(e_{add}\) is the fraction according to \(e_{add}=a*e_{g}=a*(\sum_{i\in{T}}k(i) + \sum_{j\in{S}}k(j))\), with \(a\in{(0,1)}\), and \(e_{g}=(\sum_{i\in{T}}k(i) + \sum_{j\in{S}}k(j))\) the total of edges in both groups. Any two members of different groups are then randomly connected until the added connections reaches \e_{add}\).  
