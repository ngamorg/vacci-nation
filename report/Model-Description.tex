...
The Watts-Strogatz graph for every group \(G\subset{N}\) is then built by taking into account \(k\), the number of nearest neighbours that are connected to in a ring topology. \\ 

STARTING HERE:
Then, the two graphs are combined for any two members \(i,j\in{N}\) of different groups, requiring \(g(i)\neq g(j)\). The expected amount of edges \(e\) is added according to \(e=a*n\), with \(a\in{(0,1)}\). 
NOT SURE ABOUT CORRECT RULE - UPDATE NEEDED \\
